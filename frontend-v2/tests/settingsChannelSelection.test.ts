...
test("settings page lets admins select active LLM channels by purpose", () => {
  ...
  assert.match(source, /http:\/\/43\.173\.105\.8:8080\/v1/, "settings should use the configured kakayiduo base URL");
  assert.match(source, /gpt-5\.5/, "settings should expose gpt-5.5");
  assert.match(source, /gpt-5\.4-mini/, "settings should expose gpt-5.4-mini");
