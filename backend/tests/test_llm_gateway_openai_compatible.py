...
class OpenAICompatibleGatewayTest(unittest.TestCase):
    ...
    def test_gateway_uses_database_default_chat_provider_before_environment(self) -> None:
        provider = GatewayProvider(
            capability="chat",
            name="DB Local Model",
            provider="openai_compatible",
            base_url="http://db-provider.local/v1",
            api_key="db-key",
            model="db-qwen",
            is_enabled=True,
            is_default=True,
        ...
        def fake_post(url, headers, json, timeout):
            ...
            return httpx.Response(
                ...
                json={
                    "model": "db-qwen",
                    "choices": [{"message": {"content": '{"answer": "db"}'}}],
        ...
        with patch("app.llm.llm_gateway.httpx.post", side_effect=fake_post):
            result = LLMGateway(settings=settings).generate(
                db=self.db,
                request=LLMGatewayRequest(module_name="account_package", user_prompt="user"),
            )

        self.assertTrue(result.success)
        self.assertEqual(result.provider, "openai_compatible")
        self.assertEqual(result.model, "db-qwen")
        self.assertEqual(result.data, {"answer": "db"})
        self.assertEqual(calls[0]["url"], "http://db-provider.local/v1/chat/completions")
        self.assertEqual(calls[0]["headers"]["Authorization"], "Bearer db-key")
        self.assertEqual(calls[0]["json"]["model"], "db-qwen")
