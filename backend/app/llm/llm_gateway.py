...
class LLMImageGatewayRequest(BaseModel):
...
class LLMGateway:
    ...
    def generate(
        ...
        elif provider in {"openai_compatible", "moyu"}:
    ...
    def generate_image(
        ...
        project_id: int | None = None,
        user_id: int | None = None,
        prompt_version: str | None = "image-v1",
    ) -> LLMGatewayResponse:
        started_at = time.perf_counter()
        provider = "moyu_image"
        model = self.settings.llm_image_model
        base_url = self.settings.llm_image_base_url.strip()
        ...
        if not base_url:
            result = self._error_response(
                ...
                error="LLM_IMAGE_BASE_URL is required for image generation",
            )
        else:
            result = self._generate_openai_compatible_image(request, started_at)
    ...
    def _generate_openai_compatible_image(
        ...
        provider = "moyu_image"
        model = self.settings.llm_image_model
        url = self._images_generations_url(self.settings.llm_image_base_url)
        headers = {"Content-Type": "application/json"}
        api_key = self.settings.llm_image_api_key or self.settings.llm_api_key
        if api_key:
        ...
        try:
            ...
            response.raise_for_status()
            body = response.json()
            images = self._extract_image_outputs(body)
            ...
            return self._error_response(
                ...
                model=model,
                started_at=started_at,
                error=f"Image generation timed out after {self.settings.llm_timeout_seconds} seconds: {exc}",
            )
        except httpx.HTTPStatusError as exc:
            response_text = exc.response.text[:500] if exc.response is not None else ""
            return self._error_response(
                ...
                model=model,
                started_at=started_at,
                error=f"Image generation HTTP error {exc.response.status_code}: {response_text}",
            )
        except httpx.RequestError as exc:
            return self._error_response(
                provider=provider,
            ...
            return self._error_response(
                ...
                model=model,
                started_at=started_at,
                error=f"Image generation response parse failed: {exc}",
            )
    ...
    def _images_generations_url(self, base_url: str) -> str:
        cleaned = base_url.rstrip("/")
        if cleaned.endswith("/images/generations"):
            return cleaned
        return f"{cleaned}/images/generations"

    def _extract_image_outputs(self, body: dict[str, Any]) -> list[dict[str, Any]]:
        data = body.get("data")
