...
type VideoMode = 'image' | 'text' | 'reference'
...
const prompt = ref('一位美女戴着一个满绿的福瓜在湖边散步，微风吹过，轻轻晃动着她的裙摆，她婀娜多姿，轻声哼唱孙燕姿的绿光')
const quality = ref<VideoQuality>('720P')
...
const qualityOptions: VideoQuality[] = ['720P', '1080P']
...
function applyPromptFromRoute() {
  const queryPrompt = route.query.prompt || route.query.video_prompt || route.query.seeddance_prompt
...
function handleGenerate() {
  ...
  if (activeMode.value !== 'text' && referenceFiles.value.length < 1) {
    ElMessage.warning(activeMode.value === 'image' ? '图生视频至少上传 1 张图' : '参考生视频至少上传 1 个参考素材')
    return
  }

  generating.value = true
  generationStatus.value = '视频生成任务已创建，当前为前端界面预览，后端接口接入后会显示真实进度。'

  window.setTimeout(() => {
    generating.value = false
    ElMessage.success('视频生成界面流程已跑通')
  }, 800)
}
      ...
      <div class="video-workspace">
        <article class="video-form-panel">
          ...
          <div class="video-reference-shell">
            ...
            <div v-if="referenceFiles.length" class="reference-file-grid">
              <div v-for="file in referenceFiles" :key="file.id" class="reference-file">
                <img v-if="file.type === 'image'" :src="file.preview" :alt="file.name" />
                <video v-else :src="file.preview" muted />
                <button type="button" @click="removeReference(file.id)">移除</button>
              </div>
            </div>
          </div>
          ...
          <el-form label-position="top" class="video-form">
            <el-form-item label="提示词 *">
              <el-input
                ...
                maxlength="2500"
                show-word-limit
                placeholder="描述主体、场景、动作、镜头运动、光线、情绪和风格"
              />
            </el-form-item>
            ...
            <el-form-item label="清晰度">
              <div class="video-choice-grid two">
                <button
                  ...
                  :key="item"
                  type="button"
                  :class="['video-choice', { 'is-selected': quality === item }]"
                  @click="quality = item"
                >
                  {{ item }}
                </button>
        ...
        <aside class="video-preview-panel">
          <div class="video-preview-head">
            ...
            <el-tag class="module-purple">{{ quality }}</el-tag>
          </div>
